...
<!-- fr:journal kind=finding scope=plan id=f9-reseed-marker-hardcoded created=2026-07-23T19:41:57 phase=6 state=fixed -->
### f9-reseed-marker-hardcoded · finding [fixed] · Re-seed smoke assertion hardcoded the marker suffix too (phase 6)

After the knob assertion passed, the version-aware re-seed block failed on '✗ live seed marker not restored after re-seed': it grepped the restored marker for the literal 'autocontinue1', which I renamed to 'nopatches1' when retiring the patch. The re-seed MECHANISM works (the sentinel-removal assertion passed) - only the hardcoded suffix was stale. Fixed by asserting the restored LIVE marker EQUALS the seed marker (docker exec cat /opt/hermes-agent/.seed-version) rather than any literal string - the suffix records the patch set and legitimately changes, so pinning it is what made this go stale. Same class as f8: an assertion pinned to a mutable detail of the local patch set.
